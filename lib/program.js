...
Program.prototype.initForPath = function(path) {
    ...
    return DESCRIPTORS.programForPath(path).then(function(descriptor) {
        ...
        return DESCRIPTORS.programForPath(PATH.join(PATH.dirname(self.path), "program.local.json")).then(function(localDescriptor) {
            self.localDescriptor = localDescriptor;

            return DESCRIPTORS.programForPath(PATH.join(PATH.dirname(self.path), "program.rt.json")).then(function(runtimeDescriptor) {
...
Program.prototype.walkPackages = function(options, callback) {
    ...
    function walkPackage(parentPkg, pkgInfo, level, context) {
        ...
        return Q.when(callback(parentPkg, pkgInfo, callbackContext), function(cont) {
            ...
            Object.keys(pkgInfo[0].children.packages).sort(function(a, b) {
                return pkgInfo[0].children.packages[a][2][0] - pkgInfo[0].children.packages[b][2][0];
            });
